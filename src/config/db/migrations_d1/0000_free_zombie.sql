...
--> statement-breakpoint
CREATE INDEX `idx_footer_link_status_group_sort` ON `footer_link` (`status`,`group`,`sort`);--> statement-breakpoint
CREATE INDEX `idx_footer_link_locale_status` ON `footer_link` (`locale`,`status`);--> statement-breakpoint
CREATE TABLE `order` (
	`id` text PRIMARY KEY NOT NULL,
